...
### `uk_aq_cloudflare_scheduler_dispatchers_deploy.yml`
- Trigger: push to `main` affecting `cloudflare/scheduler-dispatchers/ingest/**`, or manual dispatch.
- Purpose: deploy the ingest phase-2 Cloudflare dry-run scheduler worker and set its secrets.
- Worker: `uk-aq-scheduler-ingest` (`cloudflare/scheduler-dispatchers/ingest`).
