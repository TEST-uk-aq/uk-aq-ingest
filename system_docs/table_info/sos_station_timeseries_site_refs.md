...
then joins the station's timeseries to its canonical observed property through
`uk_aq_core.observed_property_mappings`.

Only the archive pollutants `pm25`, `pm10`, and `no2` are eligible for this
mapping. Non-target VOC/hydrocarbon labels are ignored even if the SOS station
has other AQ timeseries.
